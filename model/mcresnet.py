...
class MCRESNET(VSR):
    def __init__(self):
        ...
        self.save_dir='./checkpoint/mcresnet'
        self.log_dir='./mcresnet.txt'
    ...
    def forward(self, frames_lr, is_training=True, reuse=False):
        ...
        #frame_0up_ref = zero_upsampling(frame_ref_y, scale=self.scale)
        ...
        with tf.variable_scope('srmodel', reuse=tf.AUTO_REUSE) as scope_sr:
                with slim.arg_scope([slim.conv2d], activation_fn=prelu , stride=1,
                    ...
                    conv1=[slim.conv2d(rnn_input[:,:,:,i:i+1], 64, [5, 5], scope='enc1_{}'.format(abs(i-idx0))) for i in range(num_frame)]
                    merge=tf.concat(conv1,-1)
                    #merge=slim.conv2d(rnn_input, 64, [5, 5], scope='merge')
                    conv2=merge
                    for i in range(9):
                        conv2=slim.conv2d(merge, 32, [3, 3], scope='enc2_{}'.format(i))
                        if i !=0:
                            merge+=conv2
                        else:
                            merge=conv2
...
if __name__ == '__main__':
    model = MCRESNET()
    #model.train()
    #model.testvideos('/dev/f/data/video/test2/vid4', 0, savename='mcresnet1')
